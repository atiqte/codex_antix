...
class FreshVMSetupTests(unittest.TestCase):
    ...
    def make_package_tools_ready(self) -> None:
        for name in ("git", "curl", "rsync", "mbsync", "sv", "svlogd", "ssh", "ss", "unzip", "tar", "sha256sum"):
    ...
    def make_mbsync_ready(self) -> None:
        ...
        (self.mail / "Mailstore" / "mbsync" / "provider-live").mkdir(parents=True)
        self.write_executable(
    ...
    def write_notmuch_mock(self, ignore_local: bool = False, fail_new: bool = False) -> None:
        ignore = "local-maildir" if ignore_local else "provider-inbox-test\ntest-maildir"
