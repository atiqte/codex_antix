...
func TestApplicationJavaScriptServedLocally(t *testing.T) {
	...
	js := res.Body.String()
	for _, want := range []string{"data-pane-divider", "notmuch-browser.result-pane-height-percent", "event.clientY", "bounds.height", "ArrowUp", "ArrowDown", "[data-copy-target], [data-copy-text]", "button.dataset.copyText", "button.dataset.copyFeedbackMs", "feedbackDuration"} {
