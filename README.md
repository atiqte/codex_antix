...
- `scripts/notmuch_browser_fresh_vm_setup.sh` is the release-pinned, state-aware beginner assistant for a blank antiX VM. After the owner prepares a dedicated XFS `/mail`, one `guided-install` command verifies a private canonical recovery set, installs exact prerequisites, securely configures mbsync/notmuch, restores every approved source, performs the long resumable first index, installs the full browser, enables user runit, and validates localhost health. `scripts/notmuch_browser_recovery_kit.sh` builds and verifies the private historical-plus-delta recovery set. The stable tag remains deliberately unpublished until the canonical recovery set and a disposable antiX/Windows pilot pass.
