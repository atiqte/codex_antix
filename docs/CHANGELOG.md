...
- Added bounded Word/Outlook detection and Office-only Aptos 10.5pt/1.35 normalization while preserving sender layout, imagery, colors, tables, explicit small text, and all existing sanitization/CSP controls.
- Passed Go/Python/vet/shell/build, deterministic Tailwind, responsive browser, clean-bundle, and asset-budget validation; prepared the exact 32-member no-font Outlook Readable source bundle for staged antiX rollout.
- Added a standalone fail-closed antiX isolated validator that binds exact build/process identities, checks Readable/Original and ten-CID behavior, retains successful candidates without an interactive prompt, and limits failure cleanup to the matching candidate process.
