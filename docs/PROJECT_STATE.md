...
- `docs/NOTMUCH_ASTROID_ANTIX_GUIDE.html`, an offline browser DIY guide with copy buttons for the validated notmuch pilot search layer, full Chunk 8A browser-viewer bootstrap, full Chunk 8B/8B-FIX1/8C helper and reindex validation flow, daily browser-viewer start/stop, Astroid limitations, backup/restore, and pilot-safe reindexing.
...
- `docs/NOTMUCH_ASTROID_ANTIX_GUIDE.html` and `.txt` now document the validated pilot path, including the full Chunk 8A command that creates `~/.local/bin/notmuch-browser-pilot` from scratch on another antiX VM, Chunk 8B helper creation, Chunk 8B-FIX1 corrected helper/tag validation, Chunk 8C pilot-safe reindex validation, daily browser-viewer start/stop commands, status checks, CLI search examples, tag backup/restore, Astroid experimental use, and the deferred full-archive expansion preflight.
