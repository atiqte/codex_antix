...
- A manual desktop-popup test on 2026-07-12 found `notify-send` installed but no `org.freedesktop.Notifications` service in the IceWM session. The controller already treats popup delivery as optional, so persistent threshold alerts, daily reminder state, logs, and status remain operational. A notification daemon must be discovered/installed and enrolled in IceWM startup before visual popups can be considered validated.
