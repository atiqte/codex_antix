...
- A manual desktop-popup test and discovery audit on 2026-07-12 found a healthy IceWM session D-Bus and `notify-send 0.8.6`, but no owner/service/process/startup entry for `org.freedesktop.Notifications`. The controller already treats popup delivery as optional, so persistent threshold alerts, daily reminder state, logs, and status remain operational. Debian Trixie offers `dunst 1.12.2-1`; it is the selected lightweight, desktop-independent daemon to install and validate next.
