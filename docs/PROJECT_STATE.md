...
- Next validation must copy the verified export package from USB or from the verified Win11 host-share package into `/mail/import-staging/maildirpp-archive-export-20260701-215204` on antiX, then run antiX-side `verify-archive`, unpack, verify-tree, inspect, and add to Evolution as `Maildir-format mail directories`.
...
3. Copy the verified export package from USB or from the verified Win11 host-share package into `/mail/import-staging/maildirpp-archive-export-20260701-215204` on antiX, preserving `manifest.json`, `inventory.jsonl`, and all 21 `maildirpp-archive.tar.gz.partNNNN` files.
