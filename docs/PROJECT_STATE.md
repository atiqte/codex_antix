...
- The 15-minute provider-live archive threshold monitor and IceWM startup block were installed and validated on antiX on 2026-07-11. Repeated startup installation left exactly one marked block with mode 700. The simulation was non-persistent; the real check recorded 915 messages below the 15,000 threshold. Monitor pid 24481 runs the exact installed `monitor-loop` command at 900 seconds with alert clear and transaction lock absent. Archive/tmp remain empty, isyncrc/notmuch hashes are unchanged, and mbsync/index/browser services remain healthy. A future VM login/reboot should still prove actual IceWM activation.
