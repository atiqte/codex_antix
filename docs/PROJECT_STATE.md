...
- Next validation must unpack the verified antiX staging package from `/mail/import-staging/maildirpp-archive-export-20260701-215204/manifest.json` into `/mail/Mailstore/evolution/local-maildir`, then run `verify-tree`, run `inspect`, and add it to Evolution as `Maildir-format mail directories`.
...
3. Keep the verified staging package at `/mail/import-staging/maildirpp-archive-export-20260701-215204` until antiX restore, `verify-tree`, `inspect`, and Evolution GUI validation are complete.
4. On antiX, unpack into `/mail/Mailstore/evolution/local-maildir`, run `verify-tree`, and run `inspect`.
