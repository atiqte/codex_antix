...
- Next validation must transfer the verified export `/home/atiq/maildirpp-archive-export-20260701-215204` to antiX using the documented USB path or the now-writable Win11 host-share path after a fresh identity/write probe, then run antiX-side `verify-archive`, unpack, verify-tree, inspect, and add to Evolution as `Maildir-format mail directories`.
...
3. Run the Fedora transfer preflight, then transfer `/home/atiq/maildirpp-archive-export-20260701-215204` from Fedora to `/mail/import-staging/maildirpp-archive-export-20260701-215204` on antiX by USB or by the Win11 host-share path after the mount identity and normal-user write probes pass, preserving `manifest.json`, `inventory.jsonl`, and all 21 `maildirpp-archive.tar.gz.partNNNN` files.
