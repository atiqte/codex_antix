...
- [x] Run, review, privacy-record, and preserve Gate 11: the responsive candidate passed every automated source/date/security check and stayed isolated on 8876, but manual full-message review rejected its document-scrolling sidebar.
- [x] Reproduce the Gate 11 full-message sidebar movement against the retained binary, make the desktop sidebar viewport-sticky while preserving the mobile fixed drawer, add regression coverage, pass the full suite/build, and prove fixed geometry after actual document scrolling.
- [x] Run, review, privacy-record, commit, and push Gate 12: identity-stop only the rejected Gate 11 process, preserve its evidence, clean-build the exact sticky-sidebar binary, repeat all source/date/security/read-only checks, and retain it on localhost 8876.
- [x] Obtain owner-reported antiX GUI acceptance for Gate 12: after a hard refresh, a long message opened in a new tab kept the expanded and collapsed black sidebar fixed and full-height through top/middle/bottom scrolling.
- [x] Add safe per-address `mailto:` links and adjacent raw-address copy controls to opened-message From, To, Cc, and nonempty Bcc fields; keep malformed headers plain, search senders unchanged, and pass the complete local suite/build.
- [x] Run, review, privacy-record, commit, and push Gate 13: bind the reboot-stale Gate 12 evidence, clean-build the address-link candidate on localhost 8876, and repeat source/date/security/read-only/production-isolation checks.
- [ ] Obtain antiX and Windows SSH-tunnel GUI acceptance for the Gate 13 address-link candidate, including default mail-composer opening, raw-address clipboard content, multiple-recipient wrapping, optional Bcc, and the previously accepted result/sidebar behavior.
...
- [x] Replace pasted operator chunks with a committed project-local runner plus an ignored `current.sh`, private millisecond-stamped logs, batch/log hashes, timestamps, and exit-status capture.
- [x] Review the first combined-migration preflight log: repository/tool/mount/runsvdir/production identity/listener checks passed, then an indentation-damaged Python `-c` parser stopped the read-only gate before health/config/lock/temp completion; no state changed.
- [x] Review `combined_migration_read_only_preflight_fix1`: it exposed a Linux 15-character `comm` collision where the index-control shell and browser both appeared as `notmuch-browser`; exact executable/command-line/listener audit proved one healthy browser and one index loop with no candidate listener.
- [x] Run and review the corrected project-local read-only preflight using exact full command-line and executable identity; every repository/tool/mount/runsvdir/production/health/config/lock/temp/marker assertion passed with exit 0.
- [x] Review isolated clean-build attempt 1: it stopped before creating a build directory because the batch omitted one `6f` pair from the expected production browser SHA256; actual production matched the durable validated hash and remained healthy.
- [x] Run and review isolated clean-build FIX1 with the corrected production hash: frozen dependencies, deterministic generation, Go unit/vet/race, 46 Python tests, shell/size gates, exact binary and evidence manifests all passed with production unchanged.
- [x] Start only retained candidate `c7d14e0e...` on loopback 8876 with isolated project-local temp/log, run automated HTTP/HTMX/CSP/static/message checks, and retain it only after every assertion passes; the complete gate passed before a later antiX reboot gracefully stopped the retained process.
...
- [x] Atomically install candidate `c7d14e0e...`, the two runit-aware controls, and the runit setup helper under the existing direct/IceWM supervision path, with automatic rollback on any failure and no runit staging or activation yet.
- [x] Stage only the disabled per-user runit browser/index definitions, validate exact managed/down markers and scripts, and prove no active links or change to direct production, provider-live, or IceWM startup.
- [x] Activate the staged per-user runit browser/index services with provider-live paused, automatic supervision rollback, exact symlink/runtime/health checks, and removal of only the two legacy IceWM blocks after success.
- [x] Run a separate identity-bound post-cutover audit covering the live runit processes, links, listener, routes, artifacts, startup delta, backup/pointers, logs, provider state, and unchanged repository without lifecycle actions.
- [x] Review the first strict read-only attempt: its stable baseline and route/rejection checks passed, then the retained July validator stopped before any valid capability download because it conflated inline-only part 6 with a genuine attachment; the failure trap restored index/provider state and retained the browser identity.
...
- [x] Persist and independently verify the pre-reboot baseline: exact boot/session/runsvdir/service identities, hashes, routes, health, locks, and temp state passed without lifecycle or mail/index mutation.
...
- [x] Transfer only the exact 76,778-byte bundle `50e368dc...` to antiX through a guarded incoming path, then independently verify inventory, tests, vet, race, shell syntax, and a temporary Linux build without changing production; exact Linux binary is `e68c33d1...` and production stayed unchanged.
