...
- [ ] Transfer only standalone validator `d0b3ce6c...` from the Windows checkout by SCP because the private repository's unauthenticated raw URL returns 404; independently verify it on antiX, then run exact temporary binary `e80fb11d...` on isolated port 8876 and retain it after the automated gate.
