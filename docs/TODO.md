...
- [ ] Diagnose failed Body Images Chunk 3 manual rendering from retained PID 57241: inventory affected HTML image schemes and matching MIME Content-ID/filename/location metadata without exposing bodies, then stop/clean via the private pointer before implementing relative-resource and iframe typography/spacing corrections.
- [ ] Audit the owner-provided Aptos and Iosevka SS14 ZIP archives for safe paths, licenses, valid font metadata, and available styles; then install an efficient user-local selection on antiX without committing, bundling, or serving font files.
