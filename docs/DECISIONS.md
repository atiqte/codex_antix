...
### 2026-07-28: Release-Pin One-Command Reconstruction and a Canonical Private Recovery Set

- Status: accepted
- Context: The accepted Gate 17 browser must be reproducible by a beginner on a completely new antiX runit/zzzFM-IceWM VM. Git cannot contain private mail, credentials, logs, or a copied notmuch database, and disk formatting cannot safely be hidden inside a blind installer.
- Decision: Keep disk identification, partitioning, formatting, and mounting in a separate typed-confirmation XFS guide. Start the main workflow only after `/mail` is a dedicated writable XFS mount. Run one resumable `guided-install` command from a clean `main` clone, but export and execute only an explicitly named semver release tag. Pin the stable release rather than following moving `main`. Verify and restore a private `notmuch-browser-recovery-v1` set containing the pristine 48,720-message historical split package plus the canonical 247-message Betterbird delta; obtain provider-inbox-test and provider-live through guarded deletion-safe IMAP pulls, start provider-live-archive empty, and generate four deterministic `example.test` GUI fixtures. Collect IMAP/notmuch identity through interactive prompts, keep the app password hidden and mode 600, verify exact Go 1.26.5 and Bun 1.3.14 downloads, and make partial tool/archive stages safely rerunnable. Require exact per-source path parity, rollback-protected application installation/update, localhost/read-only validation, a changed-boot validation, and a real new-mail indexing proof. Do not publish the stable tag or merge `main` until a disposable antiX VM and Windows SSH-tunnel pilot pass.
- Consequence: A beginner receives one normal setup command after the deliberately manual disk gate, while every mutable stage either reuses a verified marker, resumes a setup-owned partial restore, or stops with an actionable reason. Recovery Gate 01 finalized and twice verified the canonical set at SHA256 `6858d5050a1cfe958d43ed0dc838e4c4b4a2c495afa709ffce796327599ef9d2`; the installer and guides now pin that exact identity. The release remains an RC until an independently verified transport copy and disposable antiX/Windows pilot pass.
