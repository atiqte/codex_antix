...
- Consequence: A beginner receives one normal setup command after the deliberately manual disk gate, while every mutable stage either reuses a verified marker, resumes a setup-owned partial restore, or stops with an actionable reason. The current branch is only a release candidate: its recovery SHA is intentionally fail-closed as `PENDING_CANONICAL_RECOVERY_SET` until the logged recovery gate completes.
