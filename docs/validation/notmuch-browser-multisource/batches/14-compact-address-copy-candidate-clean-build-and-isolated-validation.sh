...
# Gate 14: preserve the accepted Gate 13 evidence, identity-stop only its
# retained process, clean-build the compact address-copy refinement, validate
# it only on localhost:8876, and retain it for final GUI review. Real addresses
# and Message-IDs stay private and are never printed to the operator log.
G13_REPO=/home/atiq/orca/workspaces/codex_antix/branch-codex
G13_COMMIT=df6aac67f681722241337a59bced6db15aa42a53
...
G13_CANDIDATE_ROOT="$G13_STATE_ROOT/multisource-compact-address-copy-candidate"
G13_POINTER="$G13_STATE_ROOT/multisource-compact-address-copy-candidate-current.env"
G13_LOG_ROOT=/mail/Logs/notmuch-browser/multisource-compact-address-copy-candidate
G13_BUILD_SCRIPT="$G13_REPO/scripts/notmuch_browser_build.sh"
G13_GATE13_BATCH="$G13_REPO/docs/validation/notmuch-browser-multisource/batches/13-message-address-links-candidate-clean-build-and-isolated-validation.sh"
G13_GATE13_LOG="$G13_REPO/docs/validation/notmuch-browser-multisource/logs/13-message-address-links-candidate-clean-build-and-isolated-validation.log"
G13_GATE13_RECORD="$G13_REPO/docs/validation/notmuch-browser-multisource/records/13-message-address-links-candidate-clean-build-and-isolated-validation.env"
G13_GATE13_POINTER="$G13_STATE_ROOT/multisource-address-links-candidate-current.env"
G13_GATE13_BATCH_SHA=f7e9f728304fbca7fc78d8e21c901da3f2189695310be13b3ce05cb0f911e183
G13_GATE13_LOG_SHA=674229acb59e59455b8922f20c621b1d6e3319be754ed4b9a95134c59c017355
G13_GATE13_RECORD_SHA=b19a9f3e5551e07d022c58a7a05ddc6f8e2bd4835c367c3a3502562ee5d97b85
G13_GATE13_POINTER_SHA=5d386a4934d3c45540fc7cca308ccd04865e62de5da594f431f06b499a28e6b5
G13_GATE13_COMMIT=86cc59bd24f1265b06764053a196a0a411a64bcb
G13_GATE13_CANDIDATE_SHA=b37b3b633188e5c1c9c34b6dbb9c58f5e2c391982437ded6f0d7efeecf68ed5b
...
G13_CSS_SHA=858e9353a67b66e616eb083cdb5fc0ee87367f69040fdafeadd43c1a860b8a90
G13_CSS_BYTES=26289
...
G13_CANDIDATE_SHA=c53c95e28989c3d9543394c8142b7b2fb199f8ebe4cbf28e13923a0444ab0692
...
for required in (
    ...
    ".message-address-entry .mini-copy-button.copied{background:var(--green-bg)",
):
    if required not in css:
        raise SystemExit("status=blocked\nreason=compact address copy CSS contract failed")
...
print("message_address_copy_feedback=pass")
PY
}

printf '%s\n' 'gate=14-compact-address-copy-candidate-clean-build-and-isolated-validation'
...
printf '%s\n' 'authorized_gate_13_candidate_identity_stop=yes'
...
g13_equal gate_13_batch_sha256 \
  "$(sha256sum "$G13_GATE13_BATCH" | awk '{print $1}')" "$G13_GATE13_BATCH_SHA"
g13_equal gate_13_log_sha256 \
  "$(sha256sum "$G13_GATE13_LOG" | awk '{print $1}')" "$G13_GATE13_LOG_SHA"
g13_equal gate_13_record_sha256 \
  "$(sha256sum "$G13_GATE13_RECORD" | awk '{print $1}')" "$G13_GATE13_RECORD_SHA"
grep -Fqx result=success "$G13_GATE13_RECORD" ||
  g13_die "Gate 13 result is not success"
grep -Fqx privacy_review=automated_and_manual_passed "$G13_GATE13_RECORD" ||
  g13_die "Gate 13 privacy review is incomplete"
g13_equal gate_13_pointer_sha256 \
  "$(sha256sum "$G13_GATE13_POINTER" | awk '{print $1}')" "$G13_GATE13_POINTER_SHA"
g13_equal gate_13_pointer_mode "$(stat -c '%a' "$G13_GATE13_POINTER")" 600
g13_equal gate_13_pointer_status \
  "$(g13_pointer_value "$G13_GATE13_POINTER" status)" retained_for_gui_review
g13_equal gate_13_pointer_commit \
  "$(g13_pointer_value "$G13_GATE13_POINTER" repository_commit)" "$G13_GATE13_COMMIT"
g13_equal gate_13_pointer_addr \
  "$(g13_pointer_value "$G13_GATE13_POINTER" addr)" 127.0.0.1:8876
g13_equal gate_13_pointer_binary_sha256 \
  "$(g13_pointer_value "$G13_GATE13_POINTER" binary_sha256)" "$G13_GATE13_CANDIDATE_SHA"
G13_GATE13_BINARY=$(g13_pointer_value "$G13_GATE13_POINTER" binary)
[[ -f "$G13_GATE13_BINARY" ]] || g13_die "Gate 13 candidate binary is missing"
g13_equal gate_13_binary_sha256 \
  "$(sha256sum "$G13_GATE13_BINARY" | awk '{print $1}')" "$G13_GATE13_CANDIDATE_SHA"
G13_CURRENT_BOOT_ID=$(sed -n '1p' /proc/sys/kernel/random/boot_id)
G13_GATE13_BOOT_ID=$(g13_pointer_value "$G13_GATE13_POINTER" boot_id)
G13_GATE13_PID=$(g13_pointer_value "$G13_GATE13_POINTER" pid)
G13_GATE13_START_TICKS=$(g13_pointer_value "$G13_GATE13_POINTER" start_ticks)
g13_equal gate_13_pointer_boot_id "$G13_GATE13_BOOT_ID" "$G13_CURRENT_BOOT_ID"
[[ "$G13_GATE13_PID" =~ ^[0-9]+$ && -d "/proc/$G13_GATE13_PID" ]] ||
  g13_die "Gate 13 retained process is not running"
g13_equal gate_13_process_executable \
  "$(readlink -f "/proc/$G13_GATE13_PID/exe")" "$G13_GATE13_BINARY"
g13_equal gate_13_process_start_ticks \
  "$(awk '{print $22}' "/proc/$G13_GATE13_PID/stat")" "$G13_GATE13_START_TICKS"
g13_equal gate_13_port_8876_listener_count \
  "$(ss -H -ltnp 'sport = :8876' | grep -F "pid=$G13_GATE13_PID," | wc -l | tr -d ' ')" 1
G13_GATE13_HEALTH=$(curl --silent --show-error --fail --max-time 10 \
  http://127.0.0.1:8876/healthz)
g13_health_to_safe_lines gate_13_before_stop "$G13_GATE13_HEALTH"
printf '%s\n' gate_13_automated_and_antix_gui_acceptance_evidence=verified
kill "$G13_GATE13_PID"
for _ in $(seq 1 100); do
  [[ ! -d "/proc/$G13_GATE13_PID" ]] && break
  sleep 0.1
done
[[ ! -d "/proc/$G13_GATE13_PID" ]] ||
  g13_die "Gate 13 candidate did not stop after SIGTERM"
g13_equal gate_13_port_8876_listeners_after_stop \
  "$(ss -H -ltnp 'sport = :8876' | wc -l | tr -d ' ')" 0
printf '%s\n' gate_13_candidate_identity_stopped=yes
printf '%s\n' gate_13_files_and_pointer_preserved=yes

g13_equal build_script_sha256 \
  "$(sha256sum "$G13_BUILD_SCRIPT" | awk '{print $1}')" "$G13_BUILD_SCRIPT_SHA"
g13_equal compact_address_copy_css_sha256 \
  "$(sha256sum "$G13_CSS" | awk '{print $1}')" "$G13_CSS_SHA"
g13_equal compact_address_copy_css_bytes "$(wc -c < "$G13_CSS" | tr -d ' ')" "$G13_CSS_BYTES"
...
[[ ! -e "$G13_POINTER" ]] || g13_die "Gate 14 candidate pointer already exists"
[[ ! -e "$G13_CANDIDATE_ROOT" ]] || g13_die "Gate 14 candidate root already exists"
...
G13_POINTER_TMP=$(mktemp "$G13_STATE_ROOT/.multisource-compact-address-copy-candidate.XXXXXX")
{
...
printf '%s\n' status=gate_14_compact_address_copy_candidate_clean_build_and_isolated_validation_pass
