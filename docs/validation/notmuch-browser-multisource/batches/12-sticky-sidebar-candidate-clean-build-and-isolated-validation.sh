...
# Gate 12: supersede only the retained Gate 11 candidate, clean-build the
# sticky-sidebar correction, validate it on localhost:8876, and retain it for
# a new manual antiX GUI review.
G8_REPO=/home/atiq/orca/workspaces/codex_antix/branch-codex
G8_COMMIT=532c783324fbb02de83da807b766b2de360214fd
...
G8_CANDIDATE_ROOT="$G8_STATE_ROOT/multisource-sticky-sidebar-candidate"
G8_POINTER="$G8_STATE_ROOT/multisource-sticky-sidebar-candidate-current.env"
G8_LOG_ROOT=/mail/Logs/notmuch-browser/multisource-sticky-sidebar-candidate
...
G8_GATE11_BATCH="$G8_REPO/docs/validation/notmuch-browser-multisource/batches/11-responsive-result-layout-candidate-clean-build-and-isolated-validation.sh"
G8_GATE11_LOG="$G8_REPO/docs/validation/notmuch-browser-multisource/logs/11-responsive-result-layout-candidate-clean-build-and-isolated-validation.log"
G8_GATE11_RECORD="$G8_REPO/docs/validation/notmuch-browser-multisource/records/11-responsive-result-layout-candidate-clean-build-and-isolated-validation.env"
G8_GATE11_BATCH_SHA=c83a3690612db893a38b2f547ae41bc81085db1b5e4ef40050d30cce616a62ea
G8_GATE11_LOG_SHA=4fa2fb3b640e7cd44d26f7cc9389d4b8565588fa1a469924404cfe596ae88581
G8_GATE11_RECORD_SHA=32ff751ec2737124e2b9cf692d9894c17d13e7e5ac1c74a659c6fa49695d833a
G8_GATE11_POINTER="$G8_STATE_ROOT/multisource-layout-candidate-current.env"
G8_GATE11_POINTER_SHA=ae42160eb156f31fa0945c0ea8ad8fe80323f3510899b9d221be96dc94471f2f
G8_GATE11_COMMIT=a1ec21a6516b36f4a3ac89ed0e2d7d40a0dbea53
G8_GATE11_CANDIDATE_SHA=5269c53393d96db1c3fe37f1160280ef9778fd3a7c795b1cf3db0a07c9c9649f
...
G8_CSS_SHA=3c07b0c4a9331a1176a02193121ff98680ea4036387bc90f49ad745b1b6cda2a
G8_CSS_BYTES=25415
...
G8_CANDIDATE_SHA=4f33c40777b3dc49abdecb59775c8b8aa4d207dd7933349766edae4d7e5bc105
...
print("desktop_sidebar_top=0")
print("desktop_sidebar_height=100dvh")
print("mobile_sidebar_position=fixed")
PY
}

printf '%s\n' 'gate=12-sticky-sidebar-candidate-clean-build-and-isolated-validation'
...
printf '%s\n' 'authorized_gate_11_candidate_identity_stop=yes'
...
g8_equal gate_11_batch_sha256 \
  "$(sha256sum "$G8_GATE11_BATCH" | awk '{print $1}')" "$G8_GATE11_BATCH_SHA"
g8_equal gate_11_log_sha256 \
  "$(sha256sum "$G8_GATE11_LOG" | awk '{print $1}')" "$G8_GATE11_LOG_SHA"
g8_equal gate_11_record_sha256 \
  "$(sha256sum "$G8_GATE11_RECORD" | awk '{print $1}')" "$G8_GATE11_RECORD_SHA"
grep -Fqx result=success "$G8_GATE11_RECORD" || g8_die "Gate 11 result is not success"
grep -Fqx privacy_review=automated_and_manual_passed "$G8_GATE11_RECORD" ||
  g8_die "Gate 11 privacy review is incomplete"
g8_equal gate_11_pointer_sha256 \
  "$(sha256sum "$G8_GATE11_POINTER" | awk '{print $1}')" "$G8_GATE11_POINTER_SHA"
g8_equal gate_11_pointer_mode "$(stat -c '%a' "$G8_GATE11_POINTER")" 600
printf '%s\n' gate_11_automated_success_and_manual_rejection_evidence=verified
...
g8_equal sticky_sidebar_css_sha256 \
  "$(sha256sum "$G8_CSS" | awk '{print $1}')" "$G8_CSS_SHA"
g8_equal sticky_sidebar_css_bytes "$(wc -c < "$G8_CSS" | tr -d ' ')" "$G8_CSS_BYTES"
g8_validate_sidebar_css "$G8_CSS"
...
G8_OLD_PID=$(g8_pointer_value "$G8_GATE11_POINTER" pid)
G8_OLD_START_TICKS=$(g8_pointer_value "$G8_GATE11_POINTER" start_ticks)
G8_OLD_BOOT_ID=$(g8_pointer_value "$G8_GATE11_POINTER" boot_id)
G8_OLD_BINARY=$(g8_pointer_value "$G8_GATE11_POINTER" binary)
g8_equal gate_11_pointer_status \
  "$(g8_pointer_value "$G8_GATE11_POINTER" status)" retained_for_gui_review
g8_equal gate_11_pointer_commit \
  "$(g8_pointer_value "$G8_GATE11_POINTER" repository_commit)" "$G8_GATE11_COMMIT"
g8_equal gate_11_pointer_addr \
  "$(g8_pointer_value "$G8_GATE11_POINTER" addr)" 127.0.0.1:8876
g8_equal gate_11_pointer_binary_sha256 \
  "$(g8_pointer_value "$G8_GATE11_POINTER" binary_sha256)" "$G8_GATE11_CANDIDATE_SHA"
[[ -f "$G8_OLD_BINARY" ]] || g8_die "Gate 11 candidate binary is missing"
g8_equal gate_11_binary_sha256 \
  "$(sha256sum "$G8_OLD_BINARY" | awk '{print $1}')" "$G8_GATE11_CANDIDATE_SHA"
if [[ "$G8_OLD_PID" =~ ^[0-9]+$ && -d "/proc/$G8_OLD_PID" ]]; then
  g8_equal gate_11_pointer_boot_id \
    "$G8_OLD_BOOT_ID" "$(sed -n '1p' /proc/sys/kernel/random/boot_id)"
  g8_equal gate_11_process_executable \
    "$(readlink -f "/proc/$G8_OLD_PID/exe")" "$G8_OLD_BINARY"
  g8_equal gate_11_process_start_ticks \
    "$(awk '{print $22}' "/proc/$G8_OLD_PID/stat")" "$G8_OLD_START_TICKS"
  g8_equal gate_11_port_8876_listener_count \
    "$(ss -H -ltnp 'sport = :8876' | grep -F "pid=$G8_OLD_PID," | wc -l | tr -d ' ')" 1
  G8_GATE11_HEALTH=$(curl --silent --show-error --fail --max-time 10 \
    http://127.0.0.1:8876/healthz)
  g8_health_to_safe_lines gate_11_before_stop "$G8_GATE11_HEALTH"
  kill "$G8_OLD_PID"
  for _ in $(seq 1 100); do
    [[ ! -d "/proc/$G8_OLD_PID" ]] && break
    sleep 0.1
  done
  [[ ! -d "/proc/$G8_OLD_PID" ]] ||
    g8_die "Gate 11 candidate did not stop after SIGTERM"
  printf '%s\n' gate_11_candidate_identity_stopped=yes
else
  g8_equal gate_11_stale_pointer_port_8876_listeners \
    "$(ss -H -ltnp 'sport = :8876' | wc -l | tr -d ' ')" 0
  printf '%s\n' gate_11_candidate_already_stopped=yes
fi
g8_equal port_8876_listeners_after_gate_11_stop \
  "$(ss -H -ltnp 'sport = :8876' | wc -l | tr -d ' ')" 0
printf '%s\n' gate_11_files_and_pointer_preserved=yes

[[ ! -e "$G8_POINTER" ]] || g8_die "multi-source candidate pointer already exists"
[[ ! -e "$G8_CANDIDATE_ROOT" ]] || g8_die "sticky-sidebar candidate root already exists"
...
g8_validate_sidebar_css "$G8_EVIDENCE/app.css"
...
G8_POINTER_TMP=$(mktemp "$G8_STATE_ROOT/.multisource-sticky-sidebar-candidate.XXXXXX")
{
...
printf '%s\n' status=gate_12_sticky_sidebar_candidate_clean_build_and_isolated_validation_pass
