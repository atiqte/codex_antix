...
# Gate 06: guarded current-source enrollment with full rollback backup.
G6_REPO=/home/atiq/orca/workspaces/codex_antix/branch-codex
G6_COMMIT=966f3478530508024c2fb9034bae6c4e4283d1a5
...
g6_exit_report() {
  ...
  if [[ "$rc" -ne 0 && "$G6_ENROLL_STARTED" == yes ]]; then
    set +e
    printf '%s\n' gate_06_failure_recovery_observation=begin
    if [[ "$G6_HELPER_COMPLETED" == yes &&
      ...
      G6_ROLLBACK_LOG="$G6_LOG_DIR/gate06-automatic-rollback-$G6_ROLLBACK_STAMP.log"
    ...
    printf '%s\n' gate_06_failure_recovery_observation=end
  fi
  exit "$rc"
}
...
g6_indexed_files() {
  notmuch --config="$G6_CONFIG" count --output=files "path:$1/**"
}

printf '%s\n' 'gate=06-guarded-current-source-enrollment'
...
g6_equal new_tags_before \
  ...
  'inbox unread'
[[ ! -e "$G6_BACKUP_POINTER" ]] || g6_die "enrollment backup pointer already exists"
...
G6_HELPER_TRANSCRIPT="$G6_LOG_DIR/gate06-helper-$G6_STAMP.log"
...
printf '%s\n' status=gate_06_guarded_current_source_enrollment_pass
