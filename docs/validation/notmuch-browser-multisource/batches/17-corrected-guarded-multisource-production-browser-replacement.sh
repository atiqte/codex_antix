...
# Gate 17: pin Gate 16's safe pre-mutation failure, bind the exact
# cross-platform-accepted Gate 15 candidate with corrected independent
# acceptance markers, create and verify a complete production rollback backup,
# atomically replace only the browser binary under the existing per-user runit
# supervisor, validate localhost:8765, and retain Gate 15 on 8876. Real
# addresses and Message-IDs stay private.
G13_REPO=/home/atiq/orca/workspaces/codex_antix/branch-codex
G13_COMMIT=06961c37dddd56ce1f00997db1542a514a98b257
...
G13_GATE16_BATCH="$G13_REPO/docs/validation/notmuch-browser-multisource/batches/16-guarded-multisource-production-browser-replacement.sh"
G13_GATE16_LOG="$G13_REPO/docs/validation/notmuch-browser-multisource/logs/16-guarded-multisource-production-browser-replacement.log"
G13_GATE16_RECORD="$G13_REPO/docs/validation/notmuch-browser-multisource/records/16-guarded-multisource-production-browser-replacement.env"
G13_GATE16_BATCH_SHA=a64eeed593a36f978ca4de3df8531217e61d8687d273ee0bf2a98794ce0b04b1
G13_GATE16_LOG_SHA=ffcd33fec1f23e588c3ace483f701f7637cc60e53054ef2f16268a03ba667123
G13_GATE16_RECORD_SHA=0d2b1f5606b43a7768f531614aaa70e5330c4f1b58d682dac23d9f2fa4a95bba
...
printf '%s\n' 'gate=17-corrected-guarded-multisource-production-browser-replacement'
...
g13_equal gate_16_batch_sha256 \
  "$(sha256sum "$G13_GATE16_BATCH" | awk '{print $1}')" "$G13_GATE16_BATCH_SHA"
g13_equal gate_16_log_sha256 \
  "$(sha256sum "$G13_GATE16_LOG" | awk '{print $1}')" "$G13_GATE16_LOG_SHA"
g13_equal gate_16_record_sha256 \
  "$(sha256sum "$G13_GATE16_RECORD" | awk '{print $1}')" "$G13_GATE16_RECORD_SHA"
grep -Fqx result=failed "$G13_GATE16_RECORD" ||
  g13_die "Gate 16 result is not failed"
grep -Fqx privacy_review=automated_and_manual_passed "$G13_GATE16_RECORD" ||
  g13_die "Gate 16 privacy review is incomplete"
printf '%s\n' gate_16_safe_pre_mutation_failure_evidence=verified
...
grep -Fq 'antiX Gate 15 strong copied feedback GUI: PASS' \
  "$G13_REPO/docs/PROJECT_STATE.md" ||
  g13_die "antiX Gate 15 acceptance is missing from project state"
grep -Fq 'Windows 11 SSH-tunnel `127.0.0.1:8876` PASS' \
  "$G13_REPO/docs/PROJECT_STATE.md" ||
  g13_die "Windows Gate 15 acceptance is missing from project state"
...
[[ ! -e "$G13_ROLLBACK_POINTER" ]] ||
  g13_die "Gate 17 rollback pointer already exists"
[[ ! -e "$G13_PRODUCTION_POINTER" ]] ||
  g13_die "Gate 17 production pointer already exists"
[[ ! -e "$G13_ROLLOUT_ROOT" ]] ||
  g13_die "Gate 17 production rollout root already exists"
...
printf '%s\n' status=gate_17_corrected_guarded_multisource_production_browser_replacement_pass
