...
# Gate 10: rebuild after the scoped match-all fix and retain the multi-source
# candidate on localhost:8876 after pinning Gate 09's failure evidence.
G8_REPO=/home/atiq/orca/workspaces/codex_antix/branch-codex
G8_COMMIT=56a6d49a257c73e57750b4b59e0ca4439d34444b
...
G8_CANDIDATE_ROOT="$G8_STATE_ROOT/multisource-candidate"
G8_POINTER="$G8_STATE_ROOT/multisource-candidate-current.env"
G8_LOG_ROOT=/mail/Logs/notmuch-browser/multisource-candidate
...
G8_CANDIDATE_SHA=26967a5d9e7456ecfdc9350a57aaee1f736e048bfbed9c72820adc157d9f21e1
...
printf '%s\n' 'gate=10-scoped-match-all-fix-candidate-clean-build-and-isolated-validation'
...
if [[ -e "$G8_CANDIDATE_ROOT" ]]; then
  [[ -d "$G8_CANDIDATE_ROOT" && ! -L "$G8_CANDIDATE_ROOT" ]] ||
    g8_die "existing multi-source candidate root is not a real directory"
  g8_equal candidate_root_owner_before "$(stat -c '%U' "$G8_CANDIDATE_ROOT")" atiq
  printf 'preserved_failed_candidate_runs=%s\n' \
    "$(find "$G8_CANDIDATE_ROOT" -mindepth 1 -maxdepth 1 -type d -print |
      wc -l | tr -d ' ')"
else
  printf '%s\n' preserved_failed_candidate_runs=0
fi
...
printf '%s\n' status=gate_10_scoped_match_all_fix_candidate_clean_build_and_isolated_validation_pass
