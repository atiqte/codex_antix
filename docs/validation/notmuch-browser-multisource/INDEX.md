...
| 12 sticky-sidebar candidate clean build and isolated validation | success; antiX GUI PASS | `batches/12-sticky-sidebar-candidate-clean-build-and-isolated-validation.sh` (`6976acdf...`) | `logs/12-sticky-sidebar-candidate-clean-build-and-isolated-validation.log` (`0d90892a...`) | `records/12-sticky-sidebar-candidate-clean-build-and-isolated-validation.env` | Pinned Gate 11 evidence, identity-stopped only its rejected process while preserving its files/pointer, and clean-built exact corrected binary `4f33c407...`. Desktop sticky/top/100dvh and mobile fixed CSS passed both repository and served-byte checks. Read-only 31,420/51,590 health, all six source/search/date contracts, exact four-row test-maildir result, empty archive, invalid-folder rejection, security/assets/routes/temp, and unchanged production passed. The owner then reported `antiX Gate 12 sticky sidebar GUI: PASS` after the prescribed hard-refresh, long-message new-tab top/middle/bottom scroll, and collapse/expand checks. The later antiX reboot stopped PID 79328; pointer `ccfc7295...` and its files remain preserved as accepted stale evidence. |
| 13 message-address-links candidate clean build and isolated validation | success; antiX GUI PASS; visual refinement requested | `batches/13-message-address-links-candidate-clean-build-and-isolated-validation.sh` (`f7e9f728...`) | `logs/13-message-address-links-candidate-clean-build-and-isolated-validation.log` (`674229ac...`) | `records/13-message-address-links-candidate-clean-build-and-isolated-validation.env` | Bound Gate 12's accepted evidence and reboot-stale pointer, then clean-built exact 8,188,169-byte binary `b37b3b63...` from pushed commit `86cc59b...`. Full tests, 88,348-byte assets, sticky/mobile/address-wrapping CSS, read-only 31,551/51,783 health, all six source/date contracts, exact four-row test source, security/routes/temp, and unchanged production passed. One real private message rendered ten query-free mailto links with ten matching raw-address copy values; no addresses or Message-IDs entered the log. The owner reported `antiX Gate 13 address links GUI: PASS`, then requested smaller noticeable address copy controls, so PID 48951 remains accepted but must be superseded by Gate 14 before Windows review or production use. |
| 14 compact-address-copy candidate clean build and isolated validation | success; antiX size/hover/focus PASS; copied-feedback refinement requested | `batches/14-compact-address-copy-candidate-clean-build-and-isolated-validation.sh` (`dc915b94...`) | `logs/14-compact-address-copy-candidate-clean-build-and-isolated-validation.log` (`e2c0b3d9...`) | `records/14-compact-address-copy-candidate-clean-build-and-isolated-validation.env` | Verified Gate 13's exact privacy-reviewed evidence and accepted live identity, stopped only PID 48951, and preserved its files/pointer. Clean-built exact 8,192,265-byte compact-control binary `c53c95e2...` from pushed commit `df6aac6...`. Full tests, 88,863-byte assets, exact 18px control/11px icon/resting/feedback CSS, read-only 31,552/51,785 health, all six source/date contracts, exact four-row test source, real-message mailto/copy parity, security/routes/temp, and unchanged production passed. PID 70088 is retained only on 127.0.0.1:8876 under pointer `0fcdeffb...`. The owner accepted the compact size and hover/focus behavior, but requested a more noticeable copied state; Gate 15 will supersede only this exact retained process. |
| 15 strong-address-copy-feedback candidate clean build and isolated validation | success; antiX and Windows SSH-tunnel GUI PASS | `batches/15-strong-address-copy-feedback-candidate-clean-build-and-isolated-validation.sh` (`84fa9b90...`) | `logs/15-strong-address-copy-feedback-candidate-clean-build-and-isolated-validation.log` (`b89e9d83...`) | `records/15-strong-address-copy-feedback-candidate-clean-build-and-isolated-validation.env` | Verified Gate 14's exact privacy-reviewed evidence and accepted live identity, stopped only PID 70088, and preserved its files/pointer. Clean-built exact 8,192,265-byte strong-feedback binary `7f4099ed...` from pushed commit `71251c4...`. Full tests, 89,072-byte assets, exact bright/ring/pulse copied CSS, served CSS/JS hashes, 2,200ms message markup, read-only 31,552/51,785 health, all six source/date contracts, exact four-row test source, six real-message mailto/copy pairs, security/routes/temp, and unchanged production passed. PID 92012 is retained only on 127.0.0.1:8876 under pointer `6ab743ea...`. The owner reported both antiX and Windows 11 through the SSH tunnel at `127.0.0.1:8876` PASS; cross-platform GUI acceptance is complete. |
| 16 guarded multi-source production browser replacement | failed safely before mutation | `batches/16-guarded-multisource-production-browser-replacement.sh` (`a64eeed5...`) | `logs/16-guarded-multisource-production-browser-replacement.log` (`ffcd33fe...`) | `records/16-guarded-multisource-production-browser-replacement.env` | Repository and Gate 15 batch/log/record identities passed, then a brittle contiguous-phrase grep failed against differently worded project state. The batch stopped before candidate-pointer inspection, backup/rollout creation, runit stop, staging, installation, or rollback. Old production `c7d14e0e...`, absent new paths/pointers, and both healthy read-only endpoints were reconfirmed. Gate 17 will change only the acceptance-marker check and pin this failure evidence. |
| 17 corrected guarded multi-source production browser replacement | success; antiX and Windows production GUI PASS | `batches/17-corrected-guarded-multisource-production-browser-replacement.sh` (`e00d2716...`) | `logs/17-corrected-guarded-multisource-production-browser-replacement.log` (`3130a318...`) | `records/17-corrected-guarded-multisource-production-browser-replacement.env` | Pinned Gate 16's failure and Gate 15 acceptance, verified a private rollback backup, and atomically replaced old production `c7d14e0e...` with exact accepted binary `7f4099ed...`. All source/order/date/address-feedback/security/read-only contracts passed and rollback was not invoked; the 8876 fallback remained live at gate completion. A later reboot stopped that unsupervised fallback while preserving its files, and per-user runit restarted exact production on 8765 plus the index loop; direct post-reboot health, artifact, rollback, mbsync, lock, and temp checks passed. The owner then reported antiX and Windows SSH-tunnel production GUI PASS on port 8765, and the immediate identity/health audit remained exact. |
...
Gate 13's complete 228-line raw log and exact 617-line batch passed automated
credential-shape scanning plus manual review for email addresses, Message-IDs,
message headers, private message URLs, capability tokens, cookies, credentials,
private keys, and message content. Real rendered-message evidence remains only
inside the private mode-700 candidate directory; the committed log contains
counts and hashes only. The gate preserved Gate 12's accepted stale evidence,
retained the new candidate, and did not change mail, tags, the notmuch index,
configuration, or production.

Gate 14's complete 246-line raw log and exact 644-line batch passed the same
automated and manual privacy review. It identity-stopped only the accepted Gate
13 process while preserving its files/pointer, proved the compact visible
address-control CSS and all existing runtime contracts, and retained the new
candidate. No mail, tags, notmuch index, configuration, or production binary
was changed.

Gate 15's complete 260-line raw log and exact 684-line batch passed the same
automated and manual privacy review. It identity-stopped only the accepted Gate
14 process while preserving its files/pointer, proved the stronger copied-state
CSS, served JavaScript timer, 2,200ms real-message markup, and every existing
runtime contract, then retained the new candidate. No mail, tags, notmuch index,
configuration, or production binary was changed.

Gate 16's complete 27-line raw log and exact 884-line batch passed the same
automated and manual privacy review. It stopped at a project-state wording
assertion before the candidate-pointer, backup, service, installation, or
rollback sections. Production and the accepted candidate remained unchanged.

Gate 17's complete 191-line raw log and exact 907-line batch passed automated
and manual review for email addresses, Message-IDs, message headers, private
message URLs, capability tokens, cookies, credentials, private keys, and
message content. It created a checksum-verified rollback backup, atomically
installed exact accepted binary `7f4099ed...`, validated every production
contract, retained the isolated fallback at gate completion, and did not invoke
rollback. A later direct read-only audit after reboot proved that per-user runit
restarted exact production and the index loop; the unsupervised fallback
process stopped normally, while its private files and rollback evidence remain
preserved.

The owner subsequently reported `antiX and Windows Gate 17 production GUI:
PASS` against installed port 8765. The immediate read-only audit reconfirmed
the exact installed binary, sole loopback listener, healthy browser/index
supervision, current index counts, absent locks, and empty download temp. This
closes the guarded current-VM multi-source production rollout.
